...
WeightedMovingVarianceSky360::WeightedMovingVarianceSky360(bool _enableWeight,
                                    bool _enableThreshold,
                                    int _threshold) 
    : IBGS(quote(WeightedMovingVarianceSky360)),
    m_params(_enableWeight, _enableThreshold, _threshold),
    m_numProcessesParallel(12)
{
    debug_construction(WeightedMovingVarianceSky360);
...
WeightedMovingVarianceSky360::~WeightedMovingVarianceSky360()
{
    debug_destruction(WeightedMovingVarianceSky360);
}

cv::Mat WeightedMovingVarianceSky360::getBackgroundModel() {
    return img_background;
}

void WeightedMovingVarianceSky360::process(const cv::Mat &_imgInput, cv::Mat &_imgOutput, cv::Mat &img_bgmodel)
{
    if (_imgOutput.empty()) {
        _imgOutput.create(_imgInput.size(), CV_8UC1);
    }
    if (img_bgmodel.empty()) {
        img_bgmodel.create(_imgInput.size(), CV_8UC1);
    }
    if (img_background.empty()) {
        img_background.create(_imgInput.size(), CV_8UC1);
    }
    ...
    if (showOutput)
        cv::imshow(algorithmName + "_FG", _imgOutput);
#endif
}

void WeightedMovingVarianceSky360::processParallel(const cv::Mat &_imgInput, cv::Mat &_imgOutput) {
    std::for_each(
        std::execution::par,
        m_processSeq.begin(),
        m_processSeq.end(),
...
void WeightedMovingVarianceSky360::process(const cv::Mat &_inImage, 
                                    cv::Mat &_outImg, 
...
void WeightedMovingVarianceSky360::weightedVarianceMono(
        const cv::Mat &img1, 
        const cv::Mat &img2, 
        const cv::Mat &img3, 
        cv::Mat& outImg,
...
void WeightedMovingVarianceSky360::weightedVarianceColor(
        const cv::Mat &img1, 
        const cv::Mat &img2, 
        const cv::Mat &img3, 
        cv::Mat& outImg,
...
void WeightedMovingVarianceSky360::save_config(cv::FileStorage &fs) {
  fs << "enableWeight" << m_params.enableWeight;
  fs << "enableThreshold" << m_params.enableThreshold;
  fs << "threshold" << m_params.threshold;
  fs << "showOutput" << showOutput;
}

void WeightedMovingVarianceSky360::load_config(cv::FileStorage &fs) {
  fs["enableWeight"] >> m_params.enableWeight;
  fs["enableThreshold"] >> m_params.enableThreshold;
  fs["threshold"] >> m_params.threshold;
  fs["showOutput"] >> showOutput;
